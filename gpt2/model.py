...
class GPT(nn.Module):
    ...
    def __init__(self, config: GPTConfig):
        ...
        self.transformer.wte.weight = self.lm_head.weight # weight sharing scheme # (50257, 768)
        # now 'self.transformer.wte.weight' pointer to the object 'self.lm_head.weight'
        # The orginal weight matrix in 'self.transformer.wte.weight' will be discard
    ...
    def configure_optimizers(self, weight_decay, learning_rate, betas, device_type):
        ...
        print(f"{'params':<19} : {num_decay + num_nodecay:,}  (decay {num_decay:,} / no-decay {num_nodecay:,})")
        
        fused_available = 'fused' in inspect.signature(torch.optim.AdamW).parameters
        use_fused = fused_available and device_type == 'cuda'
        print(f"{'fused AdamW':<19} : {use_fused}")
